...
namespace Test.Tests.Encrypt
{
    ...
    public class NtruEncryptTest : ITest
    {
        ...
        {
            NtruParameters param = DefinedParameters.APR2011743;
            // set df1..df3 and dr1..dr3 so params can be used for SIMPLE as well as PRODUCT
            param.DF1 = DefinedParameters.APR2011743FAST.DF1;
            param.DF2 = DefinedParameters.APR2011743FAST.DF2;
            param.DF3 = DefinedParameters.APR2011743FAST.DF3;
            param.DR1 = DefinedParameters.APR2011743FAST.DR1;
            param.DR2 =  DefinedParameters.APR2011743FAST.DR2;
            param.DR3 = DefinedParameters.APR2011743FAST.DR3;
            EncryptDecrypt(param);

            param = DefinedParameters.EES1087EP2;
            // set df1..df3 and dr1..dr3 so params can be used for SIMPLE as well as PRODUCT
            param.DF1 = DefinedParameters.EES1087EP2FAST.DF1;
            param.DF2 = DefinedParameters.EES1087EP2FAST.DF2;
            param.DF3 = DefinedParameters.EES1087EP2FAST.DF3;
            param.DR1 = DefinedParameters.EES1087EP2FAST.DR1;
            param.DR2 = DefinedParameters.EES1087EP2FAST.DR2;
            param.DR3 = DefinedParameters.EES1087EP2FAST.DR3;
            EncryptDecrypt(param);

            // blake 256
            param = DefinedParameters.APR2011439;
            param.MessageDigest = Digests.Blake256;
            param.DF1 = DefinedParameters.APR2011439FAST.DF1;
            param.DF2 = DefinedParameters.APR2011439FAST.DF2;
            param.DF3 = DefinedParameters.APR2011439FAST.DF3;
            param.DR1 = DefinedParameters.APR2011439FAST.DR1;
            param.DR2 = DefinedParameters.APR2011439FAST.DR2;
            param.DR3 = DefinedParameters.APR2011439FAST.DR3;
            EncryptDecrypt(param);
        ...
        private void InvalidEncoding(NtruEncrypt ntru, NtruKeyPair kp, NtruParameters param)
        {
            try
            {
                ...
                byte[] encrypted = ntru.Encrypt(plainText, kp.PublicKey);
                IntegerPolynomial h = kp.PublicKey.H.Clone();
        ...
        private void EncodedKeysSlow()
        {
            ...
            // sparse polynomials
            param = DefinedParameters.EES1499EP1;
            ...
            kp = new NtruKeyPair(priv, pub);
            encrypted = ntru.Encrypt(plainText, kp.PublicKey);
        ...
        private void EncodedKeysFast()
        {
            ...
            NtruParameters param = DefinedParameters.APR2011743FAST;
            NtruEncrypt ntru = new NtruEncrypt(param);
        ...
        {
            NtruParameters param = DefinedParameters.EES1087EP2FAST;
            NtruEncrypt ntru = new NtruEncrypt(param);
            NtruEncrypt ntru2 = new NtruEncrypt(DefinedParameters.EES1087EP2FAST);
            byte[] passphrase = Encoding.Unicode.GetBytes("password123");
            byte[] salt = ntru.GenerateSalt();
            NtruKeyPair kp1 = ntru.GenerateKeyPair(passphrase, salt);
            NtruKeyPair kp2 = ntru.GenerateKeyPair(passphrase, salt);

            if (!kp1.Equals(kp2))
                throw new Exception("NtruEncryptTest: key pair generation test failed!");

            salt = ntru.GenerateSalt();
            NtruKeyPair kp3 = ntru.GenerateKeyPair(passphrase, salt);
